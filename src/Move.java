...
public class Move extends Task<ClientContext>
{
    ...
    private Tile alEnd = new Tile(3312, 3253, 0), varEnd = new Tile(3305, 3472, 0), airEnd = new Tile(2841, 4830, 0), earEnd = new Tile(2657, 4830, 0), firEnd = new Tile(2657, 4830, 0), bodEnd = new Tile(2521, 4847, 0), edEnd = new Tile(3054, 3443, 0), falEnd = new Tile(2985, 3294, 0);
    ...
    private int currentAltar = falAltar;
    private TilePath currentPath = ctx.movement.newTilePath(tileFalAir);
    private TilePath currentPathRev = ctx.movement.newTilePath(tileFalAir).reverse();
    private Tile currentAreaEnd = falEnd;
    ...
    public void execute()
    {
        ...
        if(banking)
        {
            if(ctx.movement.distance(ctx.movement.destination()) <= 5 || ctx.movement.distance(ctx.movement.destination()) == -1) {
                if(ctx.movement.distance(ctx.objects.select().id(bankIDs).nearest().poll()) <= 6 && !ctx.objects.select().id(bankIDs).nearest().poll().inViewport()) {
